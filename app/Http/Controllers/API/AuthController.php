...
class AuthController extends BaseController
{
    ...
    public function signup(Request $request)
    {
        ...
        $client = Clients::leftJoin('zones as z', function($join) {
                    ...
                    })->where('clients.id',$client->id)->first(['clients.id','clients.active','clients.mobile','clients.name','clients.type','image','zone_id','clients.country_id','country_code','s.value3 as country_image',
    ...
    public function login(Request $request)
    {
        ...
        $clients = Clients::where('mobile', $request->mobile)->first(['id','mobile','active','name','image','country_code','password','type'
                    ,'email',"clients.zone_id","clients.country_id",'last_login',\DB::raw("CONCAT(clients.country_code,clients.mobile) AS full_mobile")]);
    ...
    public function activate(Request $request){
        ...
        $header = $request->header('Authorization');
        $token  =  Helpers::Token($header);
        $client = Clients::where('id',$token->tokenable_id)->where('reset_code',$code)->first(['id','email','name','image','country_code','mobile','active',
