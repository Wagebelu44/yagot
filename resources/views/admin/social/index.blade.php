	...
	<div class="d-flex align-items-center">
		<div class="mr-auto">
			<ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
				...
				<li class="m-nav__item">
					...
					<span class="m-nav__link-text">{{ __('lang.social_media') }}</span>
					</a>
				</li>
			</ul>
		</div>
		<div class="m-demo__preview  m-demo__preview--btn">
			...
			<button type="button" data-toggle="modal" data-target="#add_page" class="btn btn-danger m-btn m-btn--custom btnAddCustomer" style="line-height: 15px;
				padding-bottom: 15px;"><i class="fa fa-plus"></i> {{ __('lang.add_social') }}</button>
			@endcan
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	$('body').on('click','.UpdateStats',function(){
		...
		$('.loadMSG').html('{{ __('lang.updating_status') }}');
		var thisTag = $(this);
		var id = $(this).data('id');
		$('#load').show();
		$.ajaxSetup({
		...
		$(document).on('click', '.updateDetails', function () {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					swal({title: '{{ __('lang.whoops') }}', type: "error"});
				}
			});
	
			$('.modal-title').html('{{ __('lang.edit') }} بيانات');
